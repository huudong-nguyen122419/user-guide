# A6.4 · Queue 1 — Passive → Active

> A6 · Talent Active / Passive

These are freelancers and consultants locked out of every project invitation for no good reason. **33 records** on production.
...
2. **A6.4.2** — **Read each row against [A6.3 ↗](a6-3-the-decision-rule.md).** The frequent patterns that mean "should be Active":

   - **They were never classified by a human at all.** This is the single biggest source of this queue. Open **Activity Logs**: a lone entry saying the status changed, with **no reason attached**, means the [automatic rule at approval ↗](a6-x-1-in-review-queue.md) did it — purely because they held a current job at a flagged company, including a plain **corporate** one, which is group D and out of scope. Nothing about the buy side was ever checked.
...
4. **A6.4.4** — **Work through the list, not the page.** Changing a status re-runs the filter, so rows shift under you. This is why [A6.2.6 ↗](a6-2-build-the-working-list.md) tells you to freeze the list first.
