...
Left menu → **Contracts & Invoices** → **Timesheets**.
...
## B3.1.2 · Filter down to the talent

with the **Select talents** box (type a name or email, then pick). Use the **Talents with Active Contract** toggle to see only people with a running contract.
...
Click **Sync Data** on the talent's row — this pulls in whatever the talent has just submitted.

![Sync Data button](sdrx-ts-23-admin-sync-data.png)

*B3.1.3 — ① Sync Data in the Actions column · ② Create Client Invoice.*

## B3.1.4 · Sort the list to find the work, instead of scrolling it

Two buttons sit above the table: The list runs to dozens of pages, so pair this with the page-size selector at the top right rather than paging through.
