...
a **Resource** block can be rendered, and no others. All five are the same component with a different **Display Page** passed in, so a resource never appears somewhere its Display Page does not name.
