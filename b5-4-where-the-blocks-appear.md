...
2. **B5.4.2** — **Home** — the block sits under the standing FAQ cards. Do not confuse it with those: *How to write a winning pitch* and the rest are fixed page content, not resources, and no admin setting moves them.

   ![Resource block on the talent Home page](res-14-home-block.png)

   *B5.4.2 — Display Page = Home, standard layout.*

3. **B5.4.3** — **Project list** — the bottom of the filter sidebar under **Work**.

   ![Resource block on the project list](res-15-projectlist-block.png)

   *B5.4.3 — Display Page = Project.*

4. **B5.4.4** — **Project detail** — top of the right column, beside the briefing.
