# B4.x · Edge cases

> B4 · Publish a resource (Admin)

## Edge cases

* [B4.x.1 · Published but nobody sees it](b4-x-1-published-but-nobody-sees-it.md)
* [B4.x.2 · There is no Delete](b4-x-2-there-is-no-delete.md)
* [B4.x.3 · Type is locked after saving](b4-x-3-type-is-locked-after-saving.md)
* [B4.x.4 · Errors arrive one at a time](b4-x-4-errors-arrive-one-at-a-time.md)
* [B4.x.5 · Kill switch for the whole platform](b4-x-5-kill-switch-for-the-whole-platform.md)
* [B4.x.6 · No Display Page](b4-x-6-no-display-page.md)
