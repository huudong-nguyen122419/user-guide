...
1. **6.10.1** — **Two different labels, and only one of them stops anything.** This trips people up constantly:
...
3. **6.10.3** — **Out Of Office is not a rejection.** It is the most common auto-reply and it is reporting-only, so the person stays in the run — which is right, they are just away. Do not upgrade it to Not Interested to clear it off your screen; that is a different meaning and it is permanent.

4. **6.10.4** — **Nothing reminds you.** Categorising is entirely manual: no prompt, no badge, and **no count of how many replies are still unlabelled**. If you do not work the inbox deliberately, an opt-out can sit there unread while the next send goes out. **Practical habit:** after every send, open the Inbox and clear it to zero before you do anything else. Filter the category rail to **No Categories** to find what nobody has touched. There is also no bulk categorise — thirty auto-replies is thirty separate actions. Sort by the auto chip first so the identical ones are together.

5. **6.10.5** — **Where you do it.** Open the **Inbox** tab of the campaign, pick the category rail on the left to filter, click a reply, set its category. The same reply also appears in your own Inbox (Flow 4) — labelling in either place is the same record.
