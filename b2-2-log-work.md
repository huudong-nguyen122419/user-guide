...
1. **B2.2.1** — **Go to the right week.** Use **‹** / **›** next to **Today**.

   ![Week navigation](sdrx-ts-19-talent-week-nav.png)

   *B2.2.1 — ① Today plus the ‹ › arrows change week · ② hours logged for the week.*

2. **B2.2.2** — **Drag on the hour grid** in the correct day column, from start time to end time.

   ![Dragging on the hour grid](sdrx-ts-20-talent-drag-grid.png)

   *B2.2.2 — drag vertically inside the right day column, from start to end time.*

3. **B2.2.3** — The **Record New Timesheet Entry** dialog opens — check the **Date**, correct the **start / end time**, and **a Note is mandatory** (it describes the work and is printed in the invoice appendix).

   ![Record New Timesheet Entry dialog](sdrx-ts-02-talent-log-entry.png)

   *B2.2.3 — Date · start–end time · Note (required) → Create.*

4. **B2.2.4** — Click **Create**. The entry appears on the calendar in **light purple** = **not submitted**. **Dark purple** means submitted — **locked**.

   ![Submitted and unsubmitted entries](sdrx-ts-03-talent-entry-states.png)

   *B2.2.4 — light purple = not submitted (editable, deletable) · dark purple = submitted (locked).*

5. **B2.2.5** — **Edit or delete an unsubmitted entry.** Two different actions, easy to mix up:
   ...
   ![Edit and delete an entry](tsx-01-entry-actions.png)

   *B2.2.5 — ① the ✕ delete button · ② click the entry body to edit · ③ the colour legend (submitted / pending) · ④ hours still pending submission.*

6. **B2.2.6** — **Which days can be logged?** A greyed-out day cannot be logged — **four** reasons:
