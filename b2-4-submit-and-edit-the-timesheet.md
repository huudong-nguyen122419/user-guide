...
1. **B2.4.1** — Click **Submit Timesheet** (enabled only when **pending submission > 0h**). If the dialog opens with **no submit button** and a **“Update your billing or payment details”** warning instead, the account is missing its **billing address / payment info**. Fix it in [B2.x.7 ↗](b2-x-7-billing-payment-info.md) and come back.
...
2. **B2.4.2** — The **Select Timesheet for Submission** dialog lists **one row per pending day** (not per entry — several entries on the same day are merged). Everything is **ticked by default**; at the top there is a **“N working days selected”** counter and an **Uncheck all** link. Narrow the list with the **Timesheet date** range (From → To).
...
3. **B2.4.3** — **Adjust *Hour(s) to bill*** if needed. Each row has two columns: **What counts as “edited”:** the two columns on the same row **differ**. Equal = logged hours kept as-is; different = the talent deliberately set a different billable figure.
