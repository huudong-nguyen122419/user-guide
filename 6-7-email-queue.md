# 6.7 · Email Queue

> 6 · Manage a campaign

Once the campaign is running, the **Email Queues** tab holds every queued send. It uses the same three tabs as a Marketing Email: **In Queue**, **Delivered** and **Cancel**, with **All** holding the lot. The columns are **Subject, Content, Status, Attachments, Sent Date** and **Created**, and you can filter by **Sources**, **Expected Run Date** or **Steps**.

![Campaign Email Queue](sdrx-camp-13-queue.png)

*6.7: the campaign Email Queue: tabs In Queue · Delivered · Cancel and the queue columns.*

**Heads-up:** as with Marketing Emails, the SDR **Email Queues** list can be slow or show **0 / “Couldn't load”** even when the campaign has already delivered. If it looks empty, refresh, or confirm via the [campaign Inbox (6.8)](6-8-verify-replies.md) and the contact's Conversations.
