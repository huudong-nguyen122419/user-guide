...
*A5.5.1: ① + Add contacts, top right of the tab · ② the empty state naming the bulk flow.*
...
*A5.5.2: ① the Marketing Email: 1 excluded chip · ② the row count you are working against.*

## A5.5.3 · Tick the contacts you want
...
*A5.5.7: Platform Signup. ① the tab · ② the green Platform Signup marker · ③ a personal address, Not verified · ④ a work experience carrying no address · ⑤ the line that names both unticked buckets.*

## A5.5.8 · ⚠ Read the button before you click it. It will not add everyone you ticked

Confirm reads **Add 2 of 6 contacts**, and directly above it: **Not ticked by default: 4 Missing + Invalid**. Only the **Valid** bucket is pre-ticked. Contacts with a missing or unusable address are carried into the drawer but left **unticked on purpose**, so selecting six people and adding two is the expected outcome, not a fault. Worked example: six selected → `Valid 2, Missing + Invalid 4 and Platform Signup 0` → button `Add 2 of 6 contacts`. **If you want the excluded ones anyway**, open the **Missing + Invalid** tab and tick them yourself, but fix the address first, or they will bounce ([7.7a ↗](7-7-verify-in-conversations.md)).
...
*A5.5.9: ① the toast, bottom left over the sidebar. It names both numbers that matter: how many contacts went in and which marketing email they went into, and it clears on its own after a few seconds, so read it as it appears. A count lower than what you selected is the dialog holding back contacts with a missing or invalid email.*

## A5.5.10 · Check the People tab
