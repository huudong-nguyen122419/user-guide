...
## A5.6.2 · You get told twice, in two places, and they do not say the same thing
...
*A5.6.2: ① the banner and its Undo · ② the toast, the only place the email is named.*
...
There is no removal history and no bin to fish them out of, the only way back is to add them again from scratch ([A5.5 ↗](a5-5-add-contacts.md)). On a **Select all N matching** removal that is a real cost, so read the count in the bar before you confirm, not after.
