...
the publish date carried on the card and in the **Information** column. Leave it blank and **publishing fills it in for you**, whichever way you publish. Set it yourself only when you want a specific date on the card. Known bug — it does not publish anything on its ownA date here does not flip a Draft to Published by itself — on UAT there are Drafts carrying a **past** Published At that are still sitting in Draft. Resource status has only **Draft / Published / Archived**; there is no *Scheduled*. This is **logged as a known bug**; until it is fixed, treat the switch as manual and do it yourself at [B4.4 ↗](b4-4-publish-it.md) — **do not schedule a resource and walk away.** **The date still matters for a different reason:** a Published resource with an **empty** Published At never shows on the talent Resources page — see [B4.x.1 ↗](b4-x-1-published-but-nobody-sees-it.md).
