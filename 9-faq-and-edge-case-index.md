...
| My Segments page is empty | No list assigned yet. Ask Ops (O5). |
| Can I edit a contact's phone / location / LinkedIn? | No: these are read-only for you; ask your Admin to update them. Experience is read-only (shown in yrs). |
...
| I can't find Contacts/Companies pages | By design, SDR has no Contacts/Companies; those are Ops tools (O3). Your lists and Inbox are the entry points. |
| (Ops) How do I get contacts to an SDR? | Create a list (O2) → add contacts (O3) → assign it to the SDR (O5). |
